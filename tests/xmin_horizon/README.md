...
The test also creates and validates additional real blockers when the target Postgres instance supports them:
...
Replication-slot regression checks run directly against Postgres, independent of Prometheus collection timing:
...
4. Optional blocker paths require matching Postgres configuration:
